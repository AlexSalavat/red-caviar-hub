...
// Статусы документов: Меркурий / Честный знак / Проверено
function DocStatus({ s }: { s: Supplier }) {
  const hasMercury = !!(s.docs?.mercury && (s.docs.mercury.status === "linked" || (s.docs.mercury as any).orgId));
  const cz: any = s.docs && (s.docs as any).chestnyZnak;
  const hasCZ = !!(cz && (cz.status === "linked" || cz.companyId));
...
// Безопасный линк на телеграм (юзер/ссылка)
...
export function SupplierProfileSheet({
  ...
  /** ХУКИ — ВСЕГДА ВЫЗЫВАЕМ (фикс порядка хуков) */
  ...
  const pdfUrl: string | null = React.useMemo(() => {
    const pl: any = supplier?.priceList;
    const url: string | undefined = pl?.url;
  ...
  const contactNow = React.useCallback(async () => {
    if (!supplier) return;
    const tg = tgLink(supplier.contacts?.tg);
    const wa = waLink(supplier.contacts?.whatsapp);
    if (tg) { window.open(tg, "_blank"); return; }
    if (wa) { window.open(wa, "_blank"); return; }
    await reveal();
  }, [supplier, reveal]);

  /** Если sheet закрыт или supplier не выбран — вернём null (хуки уже вызвались) */
  ...
  return (
    <div className={"fixed inset-0 z-50 " + (open ? "pointer-events-auto" : "pointer-events-none")}>
      ...
      />
      {/* sheet */}
      <div className={"absolute bottom-0 left-0 right-0 " + (open ? "sheet-open" : "sheet-enter")} role="dialog" aria-modal="true">
              ...
              {Boolean(
                supplier.contacts &&
                  (supplier.contacts.phone ||
                    ...
                    (supplier as any).contacts?.website)
              ) && (
                <section className="mt-4">
                    ...
                    <div className="mt-2 rounded-2xl border border-white/10 divide-y divide-white/10 bg-white/[.03]">
                      ...
                      {supplier.contacts?.tg && (
                        <div className="px-4 py-2 text-sm">
                          ...
                          <a href={tgLink(supplier.contacts.tg)!} target="_blank" className="hover:underline">
                            {supplier.contacts.tg}
                          </a>
                        </div>
                      )}
                      ...
                      {(supplier as any).contacts?.website && (
                        <div className="px-4 py-2 text-sm truncate">
                          ...
                          <a href={(supplier as any).contacts.website} target="_blank" className="hover:underline">
                            {(supplier as any).contacts.website}
                          </a>
                        </div>
                      )}
